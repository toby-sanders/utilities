function filters = SLprepareFilters2D(rows,cols,nScales,shearLevels,directionalFilter,quadratureMirrorFilter)
...
%See also: SLgetShearletIdxs2D,SLgetShearlets2D,dfilters,MakeONFilter
    ...
    [filters.cone1.wedge, filters.cone1.bandpass, filters.cone1.lowpass] = SLgetWedgeBandpassAndLowpassFilters2D(rows,cols,shearLevels,directionalFilter,quadratureMirrorFilter);
    if rows==cols
        filters.cone2 = filters.cone1;
    else
        [filters.cone2.wedge, filters.cone2.bandpass, filters.cone2.lowpass] = SLgetWedgeBandpassAndLowpassFilters2D(cols,rows,shearLevels,directionalFilter,quadratureMirrorFilter);
    end    
end
